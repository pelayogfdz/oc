...
async function run() {
  const dbs = [
    { tenantId: '8b52cbcd-c956-4717-a1bd-02e57386aaa2', dbName: 'neondb_officecity' },
    { tenantId: 'db5d3949-f8dd-41f6-9627-90374d55d044', dbName: 'neondb_petqro' },
    { tenantId: 'cd1e1142-ae76-46aa-b2d2-e5de02904788', dbName: 'neondb_seit' },
    { tenantId: '0d246cea-0220-4328-92b0-8a1387ce6a6d', dbName: 'neondb_pizca' }
  ];

  for (const db of dbs) {
    console.log(\`Leveling \${db.dbName}...\`);
    const prisma = new PrismaClient({ 
      datasources: { 
        db: { url: \`postgresql://postgres:caanma_postgres_secure_2026@db:5432/\${db.dbName}?sslmode=disable\` } 
      } 
    });
    ...
    try {
      ...
      \`);
      console.log(\`Inserted \${result} missing products in \${db.dbName}.\`);
    } catch (e) {
      console.error(\`Failed leveling \${db.dbName}:\`, e.message);
    } finally {
...
conn.on('ready', () => {
  ...
  conn.exec(`docker exec -i caanma-app sh -c "echo '${scriptB64}' | base64 -d > /tmp/level_all_sql2.js && node /tmp/level_all_sql2.js"`, (err, stream) => {
    if (err) throw err;
