const { Client } = require('ssh2');
const fs = require('fs');
const path = require('path');

const scriptStr = `
const { PrismaClient } = require('/app/node_modules/@prisma/client');
const prisma = new PrismaClient({ datasources: { db: { url: 'postgresql://postgres:caanma_postgres_secure_2026@db:5432/neondb_officecity?sslmode=disable' } } });

async function run() {
  const sale = await prisma.sale.findFirst({
    where: { folio: '1QUE-1228' }
  });
  
  const branchSettings = await prisma.branchSettings.findUnique({
    where: { branchId: sale.branchId }
  });
  const config = JSON.parse(branchSettings.configJson);
  const apiKey = config.facturacion?.liveKey || config.facturacion?.apiTokenLive;
  
  const Facturapi = require('/app/node_modules/facturapi').default;
  const facturapi = new Facturapi(apiKey);
  const inv = await facturapi.invoices.retrieve(sale.invoiceId);
  
  console.log('Facturapi Invoice Total:', inv.total);
}

run().catch(console.error).finally(() => prisma.$disconnect());
`;

const conn = new Client();
conn.on('ready', () => {
  const scriptB64 = Buffer.from(scriptStr).toString('base64');
  conn.exec(`docker exec -i caanma-app sh -c "echo '${scriptB64}' | base64 -d > /tmp/inspect4.js && node /tmp/inspect4.js"`, (err, stream) => {
    if (err) throw err;
    stream.on('data', d => process.stdout.write(d));
    stream.stderr.on('data', d => process.stderr.write(d));
    stream.on('close', () => conn.end());
  });
}).connect({ host: '5.78.138.167', port: 22, username: 'root', privateKey: fs.readFileSync(path.join(__dirname, 'HetznerKey.pem')) });
