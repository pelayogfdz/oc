...
async function run() {
  await fixSale('#1730', 166.60);
  await fixSale('#1732', 1671.20);
}
...
conn.on('ready', () => {
  ...
  conn.exec(`docker exec -i caanma-app sh -c "echo '${scriptB64}' | base64 -d > /tmp/userfix3.js && node /tmp/userfix3.js"`, (err, stream) => {
    if (err) throw err;
