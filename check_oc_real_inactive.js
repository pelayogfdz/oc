...
async function run() {
  ...
  console.log('Real Inactive products since July 1st:', inactiveProducts.length);
  const uniqueSkus = new Set();
  
  inactiveProducts.forEach(p => {
    if(!uniqueSkus.has(p.sku)){
      uniqueSkus.add(p.sku);
      console.log(\`- \${p.sku} | \${p.name} | \${p.updatedAt.toISOString()} | Stock: \${p.stock}\`);
    }
  });
}
