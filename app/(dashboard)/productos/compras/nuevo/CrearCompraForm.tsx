...
export default function CrearCompraForm({ suppliers, products, branchId, preloadedOrder }: { suppliers: any[], products: any[], branchId: string, preloadedOrder?: any }) {
  ...
  const scannerBufferRef = React.useRef<string>('');
  ...
  const filteredProducts = isOnline && searchTerm.trim() !== ''
  ...
  const handleImmediateSearch = useCallback(async (term: string) => {
    ...
    return false;
  }, [availableProducts, isOnline, branchId, handleAddItem]);
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      ...
      if (activeElement && (
        ...
        if (activeElement.id === 'compras-search-input') {
          // Let it fall through
        } else {
          return;
        }
