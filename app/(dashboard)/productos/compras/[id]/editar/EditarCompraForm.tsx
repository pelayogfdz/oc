...
export default function EditarCompraForm({ purchase, products, suppliers, branchId }: EditarCompraFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/productos/compras/${purchase.id}`);
      router.refresh();
    } catch (err: any) {
    ...
    <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "1rem 0" }}>
      <style>{`
        .compra-checkout-btn {
          width: 100%;
          padding: 1rem 1.5rem;
          background-color: var(--caanma-primary, #8b5cf6);
          color: white;
          border: none;
          border-radius: 8px;
          font-size: 1.1rem;
      ...
      {/* TOP ROW: Search trigger & Supplier selector */}
      <div style={{ 
        display: "grid", 
        ...
        {/* Left Side: Product Search & Category */}
        <div style={{ display: "flex", gap: "1rem", alignItems: "end", flex: 1 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem", flex: 1 }}>
            <label style={{ fontSize: "0.85rem", fontWeight: 800, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em" }}>Agregar productos</label>
            <div 
              ...
              style={{ 
                ...
                borderRadius: "8px", 
                border: "2px solid var(--caanma-primary, #8b5cf6)", 
            ...
            >
              ...
              {searchTerm || "Buscar por nombre, SKU..."}
            </div>
          </div>
          ...
          >
            ...
            {availableSuppliers.map((s) => (
              <option key={s.id} value={s.id}>
                {s.name} {s.creditLimit ? `(Crédito: $${s.creditLimit})` : ""}
              </option>
            ))}
          </select>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "1.5rem", alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", minHeight: "300px" }}>
            {items.length === 0 ? (
              <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", color: "#94a3b8", padding: "5rem 1rem", border: "1px dashed #cbd5e1", borderRadius: "12px", backgroundColor: "white" }}>
                ...
                <div style={{ fontSize: "0.85rem", marginTop: "0.25rem", color: "#94a3b8" }}>Busca artículos arriba para agregar a la compra.</div>
              </div>
            ) : (
              items.map((item, idx) => (
                <div 
        ...
        {/* RIGHT COLUMN: Payment and Freight */}
