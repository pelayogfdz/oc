  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '1rem 0' }}>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem', position: 'relative' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem', marginBottom: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 'bold', color: '#64748b', marginBottom: '0.5rem' }}>Solicitar A (Surte) <span style={{ color: 'red' }}>*</span></label>
              <select value={fromBranchId} onChange={e => setFromBranchId(e.target.value)} style={{ width: '100%', padding: '0.65rem 0.75rem', borderRadius: '8px', border: '1px solid #cbd5e1', fontWeight: 'bold', fontSize: '0.9rem', height: '42px', outline: 'none' }}>
                  ...
                  {displayedProducts.length === 0 ? (
                    ...
                    displayedProducts.slice(0, 15).map((p: any) => (
                      <div
                        key={p.id}
                        onClick={() => {
                          if (ventasConfig.venderSinStock || p.stock > 0) {
                        ...
                        style={{
                          ...
                          cursor: (ventasConfig.venderSinStock || p.stock > 0) ? 'pointer' : 'not-allowed',
                          display: 'flex',
                          justifyContent: 'space-between',
                          alignItems: 'center',
                          transition: 'background-color 0.15s',
                          opacity: (ventasConfig.venderSinStock || p.stock > 0) ? 1 : 0.5
                        }}
                        onMouseEnter={e => {
                          if (ventasConfig.venderSinStock || p.stock > 0) {
      ...
      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '1.5rem', paddingBottom: '0.5rem' }}>
        <span style={{ fontSize: '0.85rem', fontWeight: 'bold', color: '#64748b', marginRight: '0.5rem' }}>Solicitudes en Espera:</span>
        {onHoldTransfers.length === 0 ? (
          <span style={{ fontSize: '0.85rem', color: '#94a3b8', fontStyle: 'italic' }}>No hay solicitudes pendientes</span>
        ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '1.5rem', alignItems: 'start' }}>
        ...
        <div className="card" style={{ padding: '1.5rem' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 'bold', color: '#1e293b', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem', margin: 0 }}>
            Artículos Solicitados ({transferItems.length})
          </h3>
          ...
          {transferItems.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#94a3b8', border: '2px dashed #e2e8f0', borderRadius: '12px' }}>
              ...
              <p style={{ fontSize: '0.85rem', color: '#94a3b8', marginTop: '0.25rem' }}>
                Busca productos en la barra superior para agregarlos a la solicitud.
              </p>
            </div>
          ) : (
