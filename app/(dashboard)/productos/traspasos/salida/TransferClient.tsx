    ...
    try {
      ...
      if (!isOnline) {
         ...
         if (createdTransferId) {
           if (confirm('Operación realizada correctamente. ¿Deseas imprimir la etiqueta logística para el chofer?')) {
             window.open(`/productos/traspasos/${createdTransferId}/etiqueta`, '_blank', 'width=400,height=600');
           }
           if (confirm('¿Deseas imprimir etiquetas de códigos de barra para los productos transferidos?')) {
