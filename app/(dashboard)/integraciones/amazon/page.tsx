...
import AmazonCalculator from './Calculator';

export default async function AmazonConfigPage() {
  const branch = await getActiveBranch();
  
  const integration: any = await prisma.storeIntegration.findUnique({
    where: { branchId_platform: { branchId: branch.id, platform: 'AMAZON' } }
  });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 'bold' }}>Configuración de Amazon</h1>
          <p style={{ color: 'var(--pulpos-text-muted)' }}>Mapeo y simulación FBA/FBM.</p>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: '2rem', marginBottom: '2rem' }}>
        ...
        <form action={saveIntegrationTokens} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div>
            <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '0.25rem', color: 'var(--pulpos-primary)' }}>Access Token de Producción</label>
            <input 
              type="text" 
              name="accessToken"
              required
