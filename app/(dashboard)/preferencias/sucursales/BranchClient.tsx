  ...
  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem', marginBottom: '3rem' }}>
        {branches.map(b => (
          <div key={b.id} style={{ border: b.id === currentBranchId ? '2px solid var(--caanma-primary)' : '1px solid var(--caanma-border)', borderRadius: '8px', padding: '1.5rem', position: 'relative', overflow: 'hidden' }}>
            {b.id === currentBranchId && (
              <div style={{ position: 'absolute', top: 0, right: 0, backgroundColor: 'var(--caanma-primary)', color: 'white', fontSize: '0.65rem', padding: '0.25rem 0.5rem', borderBottomLeftRadius: '8px', fontWeight: 'bold' }}>
                ACTIVA (TÚ)
              </div>
            )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--caanma-text-muted)', fontSize: '0.85rem', marginBottom: '1rem' }}>
              <MapPin size={14} /> {b.location || 'Sin ubicación específica'}
            </div>
            
            <div style={{ display: 'flex', gap: '1rem', borderTop: '1px solid var(--caanma-border)', paddingTop: '1rem', fontSize: '0.85rem', color: 'var(--caanma-text-muted)' }}>
          ...
          <div className="card" style={{ width: '400px', padding: '2rem' }}>
            ...
            <form onSubmit={handleUpdate} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                <input type="text" name="name" defaultValue={editingBranch.name} required style={{ width: '100%', padding: '0.75rem', borderRadius: '6px', border: '1px solid var(--caanma-border)' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 'bold', marginBottom: '0.25rem' }}>Ubicación</label>
                <input type="text" name="location" defaultValue={editingBranch.location} style={{ width: '100%', padding: '0.75rem', borderRadius: '6px', border: '1px solid var(--caanma-border)' }} />
              </div>

              <div style={{ marginTop: '0.5rem', paddingTop: '1rem', borderTop: '1px solid var(--caanma-border)' }}>
                <h3 style={{ fontSize: '1rem', fontWeight: 'bold', marginBottom: '1rem', color: 'var(--caanma-primary)' }}>Coordenadas GPS (Asistencia)</h3>
                <div style={{ display: 'flex', gap: '1rem' }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 'bold', marginBottom: '0.25rem' }}>Latitud</label>
                    <input type="number" step="any" name="lat" defaultValue={editingBranch.hrLocation?.lat || ''} style={{ width: '100%', padding: '0.75rem', borderRadius: '6px', border: '1px solid var(--caanma-border)' }} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 'bold', marginBottom: '0.25rem' }}>Longitud</label>
                    <input type="number" step="any" name="lng" defaultValue={editingBranch.hrLocation?.lng || ''} style={{ width: '100%', padding: '0.75rem', borderRadius: '6px', border: '1px solid var(--caanma-border)' }} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 'bold', marginBottom: '0.25rem' }}>Radio (mts)</label>
                    <input type="number" step="any" name="radius" defaultValue={editingBranch.hrLocation?.radius || 50} style={{ width: '100%', padding: '0.75rem', borderRadius: '6px', border: '1px solid var(--caanma-border)' }} />
                  </div>
                </div>
              </div>
              
              <div style={{ marginTop: '0.5rem', paddingTop: '1rem', borderTop: '1px solid var(--caanma-border)' }}>
                <h3 style={{ fontSize: '1rem', fontWeight: 'bold', marginBottom: '1rem', color: 'var(--caanma-primary)' }}>Configuración de Emisión SAT (Facturapi)</h3>
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                <button type="button" onClick={() => setEditingBranch(null)} style={{ flex: 1, padding: '0.75rem', border: '1px solid var(--caanma-border)', borderRadius: '6px', background: 'white', cursor: 'pointer' }}>
                  Cancelar
                </button>
      ...
      {deleteConfirmId && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div className="card" style={{ width: '400px', padding: '2rem', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button onClick={() => setDeleteConfirmId(null)} style={{ flex: 1, padding: '0.75rem', border: '1px solid var(--caanma-border)', borderRadius: '6px', background: 'white', cursor: 'pointer' }}>
                Cancelar
              </button>
              <button 
