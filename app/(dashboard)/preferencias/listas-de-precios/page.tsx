  ...
  return (
    <div style={{ backgroundColor: 'white', borderRadius: '8px', padding: '2rem', border: '1px solid var(--caanma-border)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', borderBottom: '1px solid var(--caanma-border)', paddingBottom: '1rem' }}>
        ...
        <div style={{ padding: '0.5rem 1rem', backgroundColor: remaining === 0 ? '#fee2e2' : '#f1f5f9', color: remaining === 0 ? '#ef4444' : 'var(--caanma-text-muted)', borderRadius: '12px', fontSize: '0.875rem', fontWeight: 'bold' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {priceLists.map((list) => (
          <div key={list.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem', border: '1px solid var(--caanma-border)', borderRadius: '6px', backgroundColor: '#f8fafc' }}>
            <form action={updatePriceList} style={{ display: 'flex', flex: 1, alignItems: 'center', gap: '1rem' }}>
              ...
              <input 
                ...
                required
                style={{ padding: '0.5rem', borderRadius: '4px', border: '1px solid var(--caanma-border)', flex: 1, maxWidth: '300px' }}
              />
              <button type="submit" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', backgroundColor: 'transparent', border: '1px solid var(--caanma-text-muted)', color: 'var(--caanma-text)', borderRadius: '4px', cursor: 'pointer', fontSize: '0.875rem', fontWeight: 'bold' }}>
        ...
        {priceLists.length === 0 && (
          <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--caanma-text-muted)', backgroundColor: '#f8fafc', borderRadius: '6px', border: '1px dashed var(--caanma-border)' }}>
            No tienes listas de precios extras creadas. (Por defecto todos tienen Precio General).
          </div>
        )}

        {remaining > 0 && (
          <form action={createPriceList} style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', border: '1px dashed var(--caanma-border)', borderRadius: '6px' }}>
            <input 
              type="text" 
              name="name" 
              placeholder="Nombre de la nueva lista (Ej. VIP)" 
              required
              style={{ padding: '0.75rem', borderRadius: '4px', border: '1px solid var(--caanma-border)', flex: 1 }}
            />
