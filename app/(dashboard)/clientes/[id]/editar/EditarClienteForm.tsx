...
export default function EditarClienteForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await updateCustomerAction(id, payload);
      if (res && res.error) {
        setErrorMsg(res.error);
        setIsSubmitting(false);
        window.scrollTo({ top: 0, behavior: 'smooth' });
      } else if (res && res.success) {
        setSuccessMsg('¡Datos fiscales y perfil de cliente actualizados con éxito!');

        try {
          const { db } = await import('@/lib/offlineDB');
          const localCust = await db.customers.get(id);
          if (localCust && res.customer) {
        ...
        setTimeout(() => {
          window.location.assign(`/clientes/${id}`);
        }, 500);
      }
    } catch (err: any) {
      console.error('Error al actualizar cliente:', err);
      setErrorMsg(err.message || 'Error inesperado al guardar los cambios del cliente.');
      setIsSubmitting(false);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  };
        ...
        <div className="card" style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.5rem', marginBottom: '1.5rem' }}>
            ...
            <div>
              ...
              <input type="text" name="legalName" defaultValue={customer.legalName || ''} disabled={isGenericPublic} placeholder="Razón social oficial" style={{ width: '100%', padding: '0.75rem', borderRadius: '4px', border: '1px solid var(--caanma-border)', backgroundColor: isGenericPublic ? '#f1f5f9' : 'white' }} />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Código Postal Fiscal</label>
              <input type="text" name="zipCode" defaultValue={customer.zipCode || ''} disabled={isGenericPublic} placeholder="Código Postal" style={{ width: '100%', padding: '0.75rem', borderRadius: '4px', border: '1px solid var(--caanma-border)', backgroundColor: isGenericPublic ? '#f1f5f9' : 'white' }} />
            </div>
            <div>
