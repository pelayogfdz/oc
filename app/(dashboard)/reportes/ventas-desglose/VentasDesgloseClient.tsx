...
export default function VentasDesgloseClient({ initialData }: { initialData: any }) {
  const [data, setData] = useState(initialData);
  const [loading, setLoading] = useState(false);
  const [days, setDays] = useState(30);
  const [searchTerm, setSearchTerm] = useState('');

  const handleFilter = async (d: number) => {
    setLoading(true);
    setDays(d);
    try {
      const newData = await getSalesDetailData(d);
      setData(newData);
    } catch (e) {
      console.error(e);
    }
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          {[1, 7, 30, 90].map(d => (
            <button
              ...
              style={{
                padding: '0.5rem 1rem',
                border: `1px solid ${days === d ? 'var(--pulpos-primary)' : 'var(--pulpos-border)'}`,
                backgroundColor: days === d ? 'var(--pulpos-primary)' : 'white',
                color: days === d ? 'white' : 'inherit',
                borderRadius: '8px',
                fontWeight: '500',
        ...
        <div style={{ backgroundColor: 'white', padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--pulpos-border)' }}>
          ...
          <div style={{ marginTop: '2rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '0.5rem', borderBottom: '1px solid var(--pulpos-border)', marginBottom: '0.5rem' }}>
              <span style={{ color: 'var(--pulpos-text-muted)' }}>Tickets Totales</span>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '0.5rem', borderBottom: '1px solid var(--pulpos-border)' }}>
              <span style={{ color: 'var(--pulpos-text-muted)' }}>Total Vendido ({days}d)</span>
