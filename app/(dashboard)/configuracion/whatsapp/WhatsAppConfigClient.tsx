    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }}>
      {session?.status === 'DISCONNECTED' && (
        <>
          ...
          <p style={{ color: 'var(--caanma-text-muted)' }}>El microservicio de WhatsApp se está iniciando o necesita conectarse.</p>
          <button 
      ...
      {(session?.status === 'LOGGING_OUT' || session?.status === 'INITIALIZING') && (
        <>
          ...
          <p style={{ color: 'var(--caanma-text-muted)', textAlign: 'center', maxWidth: '380px' }}>
      ...
      {session?.status === 'QR_READY' && session?.sessionData && (
        <>
          ...
          <p style={{ color: 'var(--caanma-text-muted)', marginBottom: '1rem' }}>Abre WhatsApp en tu teléfono, ve a "Dispositivos vinculados" y escanea este código.</p>
      ...
      {session?.status === 'CONNECTED' && (
        <>
          ...
          <p style={{ color: 'var(--caanma-text-muted)', textAlign: 'center' }}>
