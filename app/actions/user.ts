...
export async function createUser(formData: FormData) {
  try {
    ...
    if (!branch) return { success: false, error: "No hay sucursal activa." };
    
    const targetBranchId = branchId || branch.id;
    if (!targetBranchId || targetBranchId === 'GLOBAL') {
      return { success: false, error: "Debes seleccionar una sucursal específica para asignar a este usuario." };
    }
    ...
    if (existingUser) {
      return { success: false, error: "El correo electrónico ya está registrado por otro usuario en el sistema." };
    }
    ...
    console.error("Error in createUser Server Action:", err);
    return { success: false, error: err.message || "Ocurrió un error inesperado al crear el usuario." };
  }
}
...
export async function updateUser(id: string, formData: FormData) {
  try {
    ...
    console.error("Error in updateUser Server Action:", err);
    return { success: false, error: err.message || "Ocurrió un error inesperado al actualizar el usuario." };
  }
}
