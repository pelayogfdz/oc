...
async function run() {
  ...
  console.log('Inserted missing products for PetQro:', result);
}
...
conn.on('ready', () => {
  ...
  conn.exec(`docker exec -i caanma-app sh -c "echo '${scriptB64}' | base64 -d > /tmp/level_petqro_sql.js && node /tmp/level_petqro_sql.js"`, (err, stream) => {
    if (err) throw err;
